...
```
docs/                  nội dung (.md)
  assets/screenshots/  ảnh editor (chèn kèm glightbox zoom)
  stylesheets/extra.css   toàn bộ custom theme
  javascripts/extra.js    micro-juice (scroll-reveal)
mkdocs.yml             config + nav + theme + palette
.github/workflows/     CI deploy (mkdocs gh-deploy)
requirements.txt       pin mkdocs-material + glightbox
```
